...
public interface ParkInfoMapper {
	public final String PARK_FIELD = "createtime,updatetime,pname,recid,devid,address,lat,lng,len,wid,price,belong,ispub";
	public final String PARK_PROP = "#{createTime},#{updateTime},#{pName},#{recId},#{devId},#{address},#{lat},#{lng},#{len},#{wid},#{price},#{belong},#{isPub}";
	...
	@Update("INSERT INTO park_info(" + PARK_FIELD + ") VALUES(" + PARK_PROP + ") ON DUPLICATE KEY "
			+ "UPDATE updatetime=#{updateTime}, pname=#{pName}, recid=#{recId}, devid=#{devId}, address=#{address}, lat=#{lat}, lng=#{lng}, len=#{len}, wid=#{wid}, price=#{price}, belong=#{belong}, ispub=#{isPub}")
